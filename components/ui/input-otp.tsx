...
function InputOTPSlot({
  ...
  return (
    <div
      ...
      className={cn(
        "relative flex size-12 items-center justify-center border-y border-e border-input bg-input/50 text-lg transition-all outline-none first:rounded-s-3xl first:border-s last:rounded-e-3xl aria-invalid:border-destructive data-[active=true]:z-10 data-[active=true]:border-ring data-[active=true]:ring-3 data-[active=true]:ring-ring/30 data-[active=true]:aria-invalid:ring-destructive/20 dark:data-[active=true]:aria-invalid:ring-destructive/40",
        className
      )}
      {...props}
    >
      {char && Number(char).toLocaleString("fa-IR")}
      {hasFakeCaret && (
